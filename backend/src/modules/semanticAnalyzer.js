...
class SemanticAnalyzer {
	...
	visitAssignmentStatement(node) {
		...
		if (symbol.kind === "const") {
			this.report(`Cannot reassign constant variable '${name}'.`, node, name);
			return "unknown";
		}
	...
	visitIdentifier(node) {
		...
		if (!symbol) {
			this.report(`Variable '${name}' is used before declaration.`, node, name);
	...
	visitBinaryExpression(node) {
		...
		if (["==", "!="].includes(operator)) {
			if (
				...
				this.report(`Cannot compare '${leftType}' with '${rightType}'.`, node);
			}
	...
	visitCallExpression(node) {
		const functionName = node.name || node.callee?.name || node.callee;
		...
		if (functionName === "toInt") {
			if (args.length !== 1) {
				this.report(
					"toInt() expects exactly one argument.",
					node,
					functionName,
				);
				return "number";
